...
struct LevelBlock GetElementByPosition(struct Gamelevel *,int,int);
void SetElementByPosition(struct GameLevel *,int,int,int,int);
int UnlockByPosition(struct GameLevel *,int,int);
int SetProperDestination(struct Gamelevel *,int,int);
...
void SetElementByPosition(struct GameLevel *LevelInfo,int x,int y,int Element,int has_bomb_flag=0){
    if(x<LevelInfo->n && x>=0 && y<LevelInfo->m && y>=0){
        if(has_bomb_flag){
...
int FillBombs(struct GameLevel *LevelInfo){
    ...
    for(counter=0;counter<LevelInfo->bombs;counter++){
        ...
        SetElementByPosition(LevelInfo,x,y,-1);
...
void LockEverything(struct GameLevel *LevelInfo){
    ...
    for (i = 0; i < LevelInfo->n; ++i) {
        for (int j = 0; j < LevelInfo->m; ++j) {
            LevelInfo->LevelReference[i][j].isLocked=1;
        }
    }
}
int Unlock_Neighborhood(struct GameLevel *LevelInfo,int x,int y,int isPressed=0) {
    if(GetElementByPosition(LevelInfo,x,y).isBomb && isPressed){
        ...
        LevelInfo->LevelReference[x][y].isLocked=0;
        if(y-1>=0 && LevelInfo->LevelReference[x][y-1].isLocked && LevelInfo->LevelReference[x][y-1].NumberInside==0 && !LevelInfo->LevelReference[x][y-1].isBomb)Unlock_Neighborhood(LevelInfo,x,y-1);
        if(y+1<LevelInfo->m &&LevelInfo->LevelReference[x][y+1].isLocked && LevelInfo->LevelReference[x][y+1].NumberInside==0 && !LevelInfo->LevelReference[x][y+1].isBomb)Unlock_Neighborhood(LevelInfo,x,y+1);
        if(x-1>=0 && y-1>=0 && y+1<LevelInfo->m && LevelInfo->LevelReference[x-1][y-1].isLocked && LevelInfo->LevelReference[x-1][y-1].NumberInside==0 && !LevelInfo->LevelReference[x-1][y-1].isBomb)Unlock_Neighborhood(LevelInfo,x-1,y-1);
        if(x-1>=0 && LevelInfo->LevelReference[x-1][y].isLocked && LevelInfo->LevelReference[x-1][y].NumberInside==0 && !LevelInfo->LevelReference[x-1][y].isBomb)Unlock_Neighborhood(LevelInfo,x-1,y);
        if(x-1>=0 && y+1<LevelInfo->m && LevelInfo->LevelReference[x-1][y+1].isLocked && LevelInfo->LevelReference[x-1][y+1].NumberInside==0 && !LevelInfo->LevelReference[x-1][y+1].isBomb)Unlock_Neighborhood(LevelInfo,x-1,y+1);
        if(x+1<LevelInfo->n && y-1>=0 && LevelInfo->LevelReference[x+1][y-1].isLocked && LevelInfo->LevelReference[x+1][y-1].NumberInside==0 && !LevelInfo->LevelReference[x+1][y-1].isBomb)Unlock_Neighborhood(LevelInfo,x+1,y-1);
        if(x+1<LevelInfo->n &&LevelInfo->LevelReference[x+1][y].isLocked && LevelInfo->LevelReference[x+1][y].NumberInside==0 && !LevelInfo->LevelReference[x+1][y].isBomb)Unlock_Neighborhood(LevelInfo,x+1,y);
        if(x+1<LevelInfo->n && y+1<LevelInfo->m && LevelInfo->LevelReference[x+1][y+1].isLocked && LevelInfo->LevelReference[x+1][y+1].NumberInside==0 && !LevelInfo->LevelReference[x+1][y+1].isBomb)Unlock_Neighborhood(LevelInfo,x+1,y+1);
